...
## Cycle Public Email Canonicalization (2026-05-18) — append-only

**Public displayed email canonical flipped:** `msanabriarea@gmail.com` → `mia@miasanabria.com` across all repo-controlled public surfaces. Single-line source change in `src/lib/mia.ts` (`MIA.contact.email`). Every public consumer reads this constant — no per-surface edits required.

Legacy `msanabriarea@gmail.com` allowance: **private/backend lead-alert routing only**. Forbidden on every public rendered surface. New regression block `scripts/audit-public-email.ts` enforces this on every `bun run audit:all`.

What's still external (not done by this cycle, by design):
- DNS / production cutover so `https://miasanabria.com` serves this Next.js build (today it still serves Mia's legacy React-SPA showing the legacy address).
- `mia@miasanabria.com` mailbox provisioning + MX records.
- GHL backend lead-routing — may continue to route to `msanabriarea@gmail.com` if that's still Mia's monitored inbox; this is the explicitly-permitted private/backend exception.

Full cycle artifact set: `docs/artifacts/cycle-public-email-canonicalization/`.

*Generated by Cycle Public Email Canonicalization lane, 2026-05-18.*
