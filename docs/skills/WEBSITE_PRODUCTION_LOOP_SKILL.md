...
last_updated_cycle: 9 (acceptance-driven visual completion — layout vs contrast separation)
...
18. **NEW v0.3.1 — Layout-acceptance gate, separate from contrast (VERIFY post-deploy)** — for any image-mode hero, layout acceptance MUST be tested as a separate axis from contrast acceptance. A hero may PASS rendered-pixel contrast (`audit:hero-contrast`) AND simultaneously FAIL layout — the cycle 8 deploy did exactly this (cream H1 readable on navy panel ≥ 14:1 contrast WHILE the H1 right-clipped on 320/375 mobile and CTAs sat below the desktop 1280/1440 fold). The gate fires per-viewport: at every required viewport (320, 375, 768, 1280, 1440), confirm (a) `[data-hero-heading]` bounding box right edge `<=` `[data-hero-copy-panel]` right edge (mobile clipping), (b) `[data-hero-cta="primary"]` bottom edge `<=` viewport_height − header_height − 24px buffer (desktop fold). Either failure blocks visual PASS regardless of contrast verdict. Token grep does not satisfy this gate — needs rendered-DOM bounding-box probe or screenshot verdict matrix + visual review.
19. **NEW v0.3.1 — Live-audit reverse-proxy pattern (VERIFY post-deploy)** — when an audit relies on CSS-injection to produce its load-bearing diff (e.g. hide-vs-normal capture), `--live` mode MUST route the live URL through a local Bun static server that proxies upstream responses and injects the audit CSS into HTML responses server-side. Pointing Chrome directly at the live URL with `?auditMode=hide` produces 0 glyph samples / all-WARN because live URLs don't honor query params for CSS injection. Cycle 8 shipped without this pattern and the `--live` mode returned 0 PASS · 95 WARN until Cycle 9 Spark Team B added the reverse proxy. Pattern lives in `scripts/audit-hero-pixel-contrast.ts` `startStaticServer(port, mutation, liveBase?)` and is reusable for any future visual sentinel.
...
21. **Mutation FAILs and mutation WARNs are both valid sentinel detection (NEW v0.3.1 — cycle 9 lesson).** When the mutation hides H1 visibility (e.g. cream-on-cream panel BG), the audit's diff produces fewer glyph samples and rows go to WARN ("low samples"), not FAIL. WARN is correct — the audit honestly reports "I can't measure contrast because the H1 isn't visible enough." The exit-code logic must treat `(WARN + FAIL) >= 10% of rows` under mutation as sentinel-success and `< 10%` as sentinel-failure. A mutation run that comes back fully green is the no-op pattern that v0.3.0 #20 was guarding against; the v0.3.1 refinement is recognizing that WARN counts as detection.
22. **Layout-acceptance and contrast-acceptance are different verdicts (NEW v0.3.1 — cycle 8 lesson).** It is structurally possible for a hero to be `audit:hero-contrast` PASS (rendered-pixel WCAG ≥ 4.5:1) AND simultaneously fail user-visible acceptance because the H1 is clipping at the viewport edge or the CTAs are below the fold. Cycle 8's GPT-5.5 live review caught this — closeout language could not say "Hero readability PASS in production" because the contrast was fine but the layout wasn't. v0.3.1 separates these as gates #13 (rendered hero readability) and #18 (layout-acceptance) so that no future cycle conflates them.
23. **Live-audit `--live` mode requires CSS-injection-via-proxy, not direct URL targeting (NEW v0.3.1 — cycle 9 root cause).** When an audit needs to compare two captures (one with the H1 visible, one with it hidden) to find the glyph mask, the hide-mode CSS injection only works through a local Bun static server that proxies the live URL. Pointing Chrome directly at the live URL with `?auditMode=hide` does nothing — live URLs don't honor query params for CSS injection. Cycle 8 shipped this anti-pattern and reported 0 PASS · 95 WARN; cycle 9 Spark Team B's reverse-proxy fix produced 95 PASS · 0 WARN against the same live URL. Pattern is the canonical shape for any future live visual audit.
24. **Heuristic verdict matrices have FALSE-POSITIVE shapes — document them (NEW v0.3.1 — cycle 9 lesson).** A pixel-cluster verdict matrix designed for one layout (Cycle 8's free-standing CTA pills) returns FAILs against a different layout (Cycle 9's panel-embedded CTAs) even when the new layout is visually correct. The matrix is a "captured-vs-reviewed" sentinel, NOT a deploy gate. Visual review + rendered-pixel audits are load-bearing. Document the matrix's known false-positive shapes in its own README so future cycles don't over-trust the verdict.
...
- **v0.3.1** — Mia cycle 9 — layout-acceptance separated from contrast-acceptance (Cycle 8 closed with contrast PASS but layout FAIL — these are different gates and must be tested separately); CTA-above-fold gate added (visual sentinel that primary CTA bottom edge is ≤ viewport_height − header_height − buffer at every desktop/laptop breakpoint); live-audit reverse-proxy pattern documented (Bun static server proxies live URL responses with audit-CSS injection — was the missing piece in Cycle 8's `--live` mode that returned 0 PASS · 95 WARN); deterministic per-route × per-viewport screenshot verdict matrix generator (`audit-screenshot-verdict-matrix.ts` — heuristic-based PASS/PARTIAL/FAIL on H1-clipping / CTA-above-fold / contrast / visual-quality; documented limitations on in-panel CTA arrangements); mutation-mode exit-code logic reframed (mutation that produces fewer than 10% non-PASS rows is itself a sentinel failure — "no-op mutation = no signal"); 19 hard gates + 3 soft gates. Driver: Cycle 8's GPT-5.5 live verdict FAIL on layout (mobile H1 right-edge clipping at 320 / 375 + desktop CTAs below the 1280 / 1440 fold) despite Cycle 8's contrast-only audit PASSING; Cycle 9 closed both with C-refined panel-embedded CTAs at lg:min-h-[560px] + lg:py-16 and `<wbr/>` proper-noun-cluster wrap hints in the locked Card-3 homepage heading. Documented residual: 320×568 primary CTA tail-clipping concern (visually appears to clip "tion" on "Conversation" at narrowest viewport even though the math says it should fit; queued for Cycle 10).
